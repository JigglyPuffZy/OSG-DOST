-- =============================================================================
-- UPSERT USIGAN CASE (Delete if exists, then insert fresh)
-- Safe to run multiple times - handles duplicates automatically
-- =============================================================================

BEGIN;

-- Delete existing Usigan case if it exists
DELETE FROM cases WHERE code = 'c-usigan-001';

-- Now insert the case fresh
WITH new_case AS (
  INSERT INTO cases (
    code,
    case_title,
...
-- Return verification
SELECT 
  '✓ SUCCESS! Usigan case inserted/updated' as message,
  c.code,
  c.case_title,
  c.case_number,
  c.status,
  (SELECT COUNT(*) FROM case_status_updates WHERE case_id = c.id) as status_updates,
  (SELECT COUNT(*) FROM case_activity WHERE case_id = c.id) as activity_entries
FROM new_case n
