-- =============================================================================
-- OSG DOST TASK FORCE — LAHAT NG QUERIES (ISANG FILE LANG)
-- =============================================================================
--
-- PAANO GAMITIN (minsan lang kailangan i-run):
--   1. Buksan ang Supabase Dashboard → SQL Editor → New query
--   2. Kopyahin ang BUONG file na ito (Ctrl+A → Ctrl+C)
--   3. I-paste sa SQL Editor → i-click ang RUN
--   4. Hintayin ang "Success" at ang message: OSG DOST schema OK
--
-- PAGKATAPOS NG SQL:
--   1. Kopyahin ang .env.example → .env sa project folder
--   2. Ilagay ang VITE_SUPABASE_URL at VITE_SUPABASE_ANON_KEY
--      (mula sa Supabase → Project Settings → API)
--   3. I-restart ang app: npm run dev
--   4. Sa Supabase: Authentication → Providers → Email
--      → i-OFF ang "Enable sign ups"
--
-- LOGIN SA APP:
...
--
-- KASAMA SA FILE NA ITO (hindi na kailangan ng ibang SQL file):
--   ✓ Tables (cases, remarks, activity, files, profiles)
--   ✓ Views (cases_full, case_stats) — ginagamit ng React app
--   ✓ 12 sample cases + 1 dummy case (c-001 DOST vs ABC)
--   ✓ Row Level Security + permissions
--   ✓ Default login user
--   ✓ Self-check (mag-e-error kung may kulang)
--
-- SAFE I-RE-RUN: buburahin at gagawin ulit ang database mula sa simula.
-- =============================================================================
-- Deep check vs the React app:--   cases.case_number and cases.court are NULLABLE (Pending / not yet assigned)
...
create table public.profiles (
  ...
  display_name    text not null default 'Mary Ann D. Carpiso',
...
-- -----------------------------------------------------------------------------
-- 9) Admin login user (removes old account, creates/updates admin)
...
begin
  ...
  raise notice 'LOGIN READY — Email: % | Password: Admin123 | Name: %', v_email, v_display_name;
end $$;

-- -----------------------------------------------------------------------------
-- 10) Final verification — dapat makita mo ito pagkatapos i-run
-- -----------------------------------------------------------------------------
select 'SETUP COMPLETE — database ready for OSG DOST app' as status;
